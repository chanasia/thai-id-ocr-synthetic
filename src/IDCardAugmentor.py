...
class IDCardAugmentor:
    ...
    def augment_image(self, image, bboxes, class_ids, class_names):
        augmented_images = []
        ...
        return augmented_images, augmented_bboxes_list, augmented_class_names_list
    
    def _save_augmented_data(self, image, bboxes, class_ids, class_names, output_name, output_images_dir, output_labels_dir):
    ...
    def process_files(self, image_files, output_dir):
        ...
        for img_path in tqdm(image_files, desc="Augmenting"):
            ...
            # Load label from base_dir/labels/
            ...
            # Augment
            aug_images, aug_bboxes_list, aug_class_names_list = self.augment_image(
                image,
                bboxes,
                class_ids,
                class_names
            ...
            for aug_idx, (aug_img, aug_bboxes, aug_class_names) in enumerate(
                zip(aug_images, aug_bboxes_list, aug_class_names_list)
            ):
    ...
    def process_dataset(self, input_images_dir, output_dir):
        """
        Legacy method - processes all images in a directory
        ...
        for img_idx, img_path in enumerate(tqdm(image_files, desc="Processing images")):
            ...
            base_name = img_path.stem
            for aug_idx, (aug_img, aug_bboxes, aug_class_names) in enumerate(zip(aug_images, aug_bboxes_list, aug_class_names_list)):
                output_name = f'{base_name}_aug_{aug_idx:03d}'
                self._save_augmented_data(
                    aug_img, 
                    aug_bboxes, 
